...
test("Basic Usage", async () => {
  ...
  const collection = client.namespace(process.env.GLACIERDB_NAMESPACE!).dataset(process.env.GLACIERDB_DATASET!).collection(collectionName);

  // model output 384-dimensional 
  const vectorStore = new GlacierVectorStore(new OpenAIEmbeddings({ modelName: process.env.GLACIERDB_MODELNAME }), {
    collection,
  });
  ...
  const results1: [Document, number][] = await vectorStore.similaritySearchWithScore(
    "Sandwich",
    1
  );
